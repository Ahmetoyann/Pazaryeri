...
class _ProductDetailScreenState extends State<ProductDetailScreen> {
      ...
      children: [
        ...
        SizedBox(
          ...
          child: FutureBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                return const Center(child: CustomLoadingIndicator(size: 50));
              }
              ...
              return ListView.separated(
                ...
                itemBuilder: (context, index) {
                  ...
                  return _buildCompactSimilarProductCard(product);
                },
              );
            },
          ),
  ...
  Widget _buildCompactSimilarProductCard(Map<String, dynamic> product) {
      ...
      children: [
        ...
        SizedBox(
          ...
          child: ElevatedButton.icon(
            ...
            label: Text(langVM.translate('add_review')),
          ),
        ),
        const SizedBox(height: 12),
        if (_reviews.isEmpty)
      ...
      children: [
        ...
        SizedBox(
          ...
          child: ElevatedButton.icon(
            ...
            label: Text(langVM.translate('ask_question_title')),
          ),
        ),
        const SizedBox(height: 12),
          ...
          Column(
            children: [
              ListView.builder(
                ...
                shrinkWrap: true,
                physics: const NeverScrollableScrollPhysics(),
                itemCount: _questions.length > 2 ? 2 : _questions.length,
                itemBuilder: (context, index) {
                    ...
                    child: OutlinedButton(
                      onPressed: () async {
                        await Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => _AllQuestionsScreen(
                              productId: widget.product['id'],
                              productName: widget.product['name'],
                              sellerName: widget.sellerName,
                            ),
                      ...
                      child: Text(langVM.translate('see_all_questions')),
                    ),
                  ),
                ),
            ],
...
class _AllReviewsScreenState extends State<_AllReviewsScreen> {
  ...
  Future<void> _loadReviews() async {
    final reviews =
        await AuthService.instance.getProductReviews(widget.productId);
    if (mounted) {
      setState(() {
        _reviews = reviews;
        _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar:
          CustomAppBar(title: Text('${widget.productName} Değerlendirmeleri')),
      body: _isLoading
          ? const Center(child: CustomLoadingIndicator())
          : _reviews.isEmpty
...
class _AllQuestionsScreen extends StatefulWidget {
  final String productId;
  final String productName;
  final String sellerName;

  const _AllQuestionsScreen({
    required this.productId,
    required this.productName,
    required this.sellerName,
  });
  ...
  @override
  State<_AllQuestionsScreen> createState() => _AllQuestionsScreenState();
}

class _AllQuestionsScreenState extends State<_AllQuestionsScreen> {
  List<Map<String, dynamic>> _questions = [];
  bool _isLoading = true;
  ...
  Future<void> _loadQuestions() async {
    final questions =
        await AuthService.instance.getProductQuestions(widget.productId);
    if (mounted) {
      setState(() {
        _questions = questions;
        _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: CustomAppBar(title: Text('${widget.productName} Soruları')),
      body: _isLoading
          ? const Center(child: CustomLoadingIndicator())
          : _questions.isEmpty
              ? Center(child: Text(langVM.translate('no_questions_yet_simple')))
              : ListView.builder(
                  padding: const EdgeInsets.all(16),
                ...
                children: [
                  ...
                  Row(
                    children: [
                      IconButton(
                        icon: const SvgIcon(
                            iconPath: AppIcons.share, color: Colors.white),
                        onPressed: _shareProduct,
                      ),
                      IconButton(
                        icon: Icon(
